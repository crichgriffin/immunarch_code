...
#'
#' @export repExplore
repExplore <- function (.data, .method = c("volume", "count", "len", "clones"), .col = c("nt", "aa"), .coding=T) {
  ...
  if (.method[1] == "volume") {
    res = add_class(data.frame(Sample = names(.data),
                               Volume = sapply(.data, function (df) {
                                 ...
                                 df %>% select(IMMCOL$count) %>% collect(n = Inf) %>% nrow()
                                 }), stringsAsFactors = F),
                    "immunr_exp_vol")

  } else if (.method[1] == "count") {
    res = lapply(1:length(.data), function (i) {
      count_table = .data[[i]] %>% select(IMMCOL$count) %>% collect(n = Inf) %>% table()
      data.frame(Sample = names(.data)[i],
             Clone.num = as.numeric(names(count_table)),
             Clonotypes = as.vector(count_table), stringsAsFactors = F)
      })
    ...
    res = lapply(.data, function (df) {
      ...
      df %>% select(seq_col) %>% collect(n = Inf) %>% sapply(nchar) %>% table()
      })
    res = lapply(1:length(.data), function (i) {
            data.frame(Sample = names(.data)[i],
                   Length = as.numeric(names(res[[i]])),
                   Count = as.vector(res[[i]]), stringsAsFactors = F)
      } )
    res = do.call(rbind, res)
    ...
    res = add_class(data.frame(Sample = names(.data),
                           Clones = sapply(.data, function (df) {
                             ...
                             df %>% select(IMMCOL$count) %>% collect(n = Inf) %>% sum(na.rm = T)
                             }), stringsAsFactors = F), "immunr_exp_clones")
  } else {
    stop("Unknown method")
    return(NULL)
  }
