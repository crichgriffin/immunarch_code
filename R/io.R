if (getRversion() >= "2.15.1") {
  utils::globalVariables(c("v_call", "d_call", "j_call", "junction", "junction_aa",
...
repLoad <- function (.path, .format = NA) {
  ...
  .read_repertoire <- function (.path, .format) {
    ...
    if (is.na(cur_format)) {
      ...
      if (suppressWarnings(is.na(parse_fun))) {
        cat("unknown format, skipping\n")
      }
      else {
        parse_res = parse_fun(.path)

        if (is.null(parse_res)) {
          cat("  [!] Warning: zero clonotypes found, skipping\n")
          parse_res = list()
        } else {
  ...
  .split_to_batches <- function (.paths, .root_folder) {
    ...
    for (folder_path in folders) {
      new_batch = .split_to_batches(dir(folder_path, full.names = T, recursive = F), folder_path)
  ...
  .check_metadata <- function (.metadata, .rep_names) {
    ...
    if (nrow(.metadata) == 0) {
      cat("  -- Metadata file not found; creating a dummy metadata...\n")
    ...
    if (length(missed_in_folders) || length(missed_in_metadata)) {
      if (length(missed_in_metadata)) {
        cat("  -- Samples found in the metadata, but not in the folder:\n     ", missed_in_metadata, "\n")
        cat("  Did you correctly specify all the sample names in the metadata file?", "\n")

        error_flag = T
      }
      if (length(missed_in_folders)) {
        cat("  -- Samples found in the folder, but not in the metadata:\n     ", missed_in_folders, "\n")
        cat("  Did you add all the necessary samples to the metadata file with correct names?", "\n")
        cat("  Creating dummy sample records in the metadata for now...", "\n")

        .metadata = merge(.metadata, as_tibble(data.frame(Sample = missed_in_folders, stringsAsFactors = F)), all = T)
    ...
    if (length(to_drop)) {
      ...
      cat("Dropping ", length(to_drop), "column(s) from .metadata.txt. Do you have spaces or tabs after the name of the last column? Remove them to ensure everything works correctly.\n")
      error_flag = T
    }
  ...
  if (nrow(.data)) {
    ...
    if (all(c("raw_clonotype_id", "barcode") %in% names(.data))) {
      .data = .data %>%
        ...
        summarise(Clones = length(unique(barcode)),
    ...
    if (any(logic)) {
      cat("  [!] Warning: found NAs in clonal counts. Setting them to 1's.\n")
      .data[[IMMCOL$count]][logic] = 1
    }
    .data = .data[order(.data[[IMMCOL$count]], decreasing = T), ]
  } else {
    .data = NULL
  }
...
parse_immunarch <- function (.filename) {
  # ToDo: add a check for a comment
  df = readr::read_tsv(.filename, skip = 1, col_types = cols())
  df = .postprocess(df)
  df
}

parse_10x_consensus <- function (.filename) {
  df = parse_repertoire(.filename, .nuc.seq = "cdr3_nt", .aa.seq = NA, .count = "umis",
...
parse_archer <- function (.filename) {
  parse_repertoire(.filename, .nuc.seq = "Clonotype Sequence", .aa.seq = NA, .count = "Clone Abundance",
