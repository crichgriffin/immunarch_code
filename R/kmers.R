...
#'
#' @export getKmers
getKmers <- function (.data, .k, .col = c("aa", "nt"), .coding = T) {
...
#'
#' kmer_profile(.data, .method = c("freq", "prob", "wei", "self"), .remove.stop = T)
#'
...
split_to_kmers <- function (.data, .k) {
  ...
  add_class(as_tibble(tab, .name_repair = function (x) c("Kmer", "Count")), "immunr_kmers")
}

kmer_profile <- function (.data, .method = c("freq", "prob", "wei", "self"), .remove.stop = T) {
  ...
  if (length(bad_symbols)) {
    warning("Warning: removed ", length(bad_symbols), " non-amino acid symbols: ", unique(bad_symbols),
