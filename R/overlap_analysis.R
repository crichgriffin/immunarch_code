...
repOverlapAnalysis <- function (.data, .method = ("hclust"), .scale = default_scale_fun, .raw=T,
                                .perp = 1, .theta = .1, .eps = .01, .k = 2) {
  if (length(.method) > 2) { stop("Can't process the .method parameter with length more than 2.") }

  .method = strsplit(.method, "+", T)[[1]]
