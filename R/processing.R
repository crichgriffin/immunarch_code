...
#'
#' @param .prob Logical. If T then sample clonotypes with probability weights equal to their number of clones. Used only if
#' ".method" is "sample".
#'
...
#' \code{.prob} argument, the function chooses clonotypes either according to their size (if \code{.prob} is T, by default),
#' or each clonotype has an equal chance to be choosed (if \code{.prob} is F). Note that sampling is done without replacing.
#'
#' @seealso \link{rmultinom}, \link{clonal_proportion}
#'
#' @export repSample
repSample <- function (.data, .method = c("downsample", "resample", "sample"), .n = NA, .prob = T) {
  .method = .method[1]
...
sample_clonotypes <- function (.data, .n, .prob) {
  ...
  indices = sample.int(nrow(.data), .n, replace = F, prob = weights)
  .data = .data[indices, ]
...
#' @examples
#' \dontrun{
#' # Create a vector with barcodes
#' barcodes = c("AAAGCAACATCACAAC-1", "GGCGACTTCATATCGG-1")
#' # Filter data and create a new list with datasets
#' your_data_filt = list()
#' your_data_filt$data = filter_barcodes(your_data$data, c("AAAGCAACATCACAAC-1", "GGCGACTTCATATCGG-1"))
#' your_data_filt$meta = your_data$meta
#' }
#'
#' @export
filter_barcodes <- function (.data, .barcodes) {
  if (!has_class(.data, "list")) {
    if (!("Barcode" %in% colnames(.data))) {
      stop('Can\'t find the "Barcode" column in the input data. Did you apply the function to a single-cell repertoire?')
    }
    found_barcodes = lapply(strsplit(.data[["Barcode"]], ";"),
                         function (x) .barcodes %in% x )
    barcode_count = sapply(found_barcodes, sum)

    .data = .data %>%
      filter(barcode_count > 0)
    barcode_count = barcode_count[barcode_count > 0]
    .data[[IMMCOL$count]] = barcode_count
    .data[[IMMCOL$prop]] = barcode_count / sum(barcode_count)
    .data
  } else {
    lapply(.data, filter_barcodes, .barcodes = .barcodes)
  }
}
