...
#' Load external immune receptor databases into R
#'
...
#' Annotate clonotypes using databases
#'
...
dbAnnotate <- function (.data, .db, .data.col, .db.col) {
  ...
  ann_res = trackClonotypes(.data, .db %>% select(.data.col), .norm = F)

  ann_res$Samples = rowSums(ann_res[, 2:ncol(ann_res)] > 0)
  ann_res = ann_res[ann_res$Samples > 0,]
  ann_res = ann_res[order(ann_res$Samples, decreasing = T),]
