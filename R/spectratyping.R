#' Analyse spectratype of immune repertoires
#'
#' @concept spectratyping
#'
...
spectratype <- function (.data, .quant = c("id", "count"), .col = "nt") {
  ...
  if (grepl("v", .col) || grepl("j", .col)) {
    ...
    setnames(.data, if (grepl("v", .col)) {"V.name"} else {"J.name"}, "Gene")
    # df = .data %>%
    #   select(Length, Count, Gene = 2) %>%
    #   group_by(Length, Gene) %>%
    #   summarise(Val = sum(Count))
    .data = as_tibble(.data)
    .data = .data[order(.data$Val, decreasing = T),]
