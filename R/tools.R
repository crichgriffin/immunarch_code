...
process_metadata_arguments <- function (.data, .by, .meta = NA, .data.sample.col = "Sample",
  ...
  # group_column = stringr::str_sort(data_groups[.data[[.data.sample.col]]], numeric = T)
  group_vec_sorted = stringr::str_sort(group_vec, numeric = T)
...
#'
#' @usage
#' apply_symm(.datalist, .fun, ..., .diag = NA, .verbose = T)
#'
#' apply_asymm(.datalist, .fun, ..., .diag = NA, .verbose = T)
#'
...
#' Working with coding and non-coding clonotype sequences
#'
#' @concept preprocessing
#'
...
coding <- function (.data) {
  if (has_class(.data, "list")) {
    ...
    d = d[grep('[*, ~]', d[[IMMCOL$cdr3aa]], invert = T), ]
    if (dt_flag) {
      data.table(d)
    } else {
      d
...
noncoding <- function (.data) {
  if (has_class(.data, "list")) {
    ...
    d = collect(.data, n = Inf)
    d = filter(d, !is.na(d[[IMMCOL$cdr3aa]]))
    d = d[grep('[*, ~]', d[[IMMCOL$cdr3aa]], invert = F), ]
    if (dt_flag) {
      data.table(d)
    } else {
      d
